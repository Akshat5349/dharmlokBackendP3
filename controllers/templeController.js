...
module.exports = {
    ...
    getAllTempleAdmin : async function(req, res){
        var temple = await templeModel.find({}).sort([['_id', 'desc']]);
        res.status(200).json({success : true,message: temple}); 
    },
    deleteTemple : async function(req, res){
    ...
    getAllTemple : async function(req, res){
        var otherTempleList = []
        var  myTempleList = []
        var otherTempleObject, myTempleObject;
        if(req.body.state == ''){
            myTempleObject = await templeModel.find({category : req.body.category, city : req.body.location, language: req.body.language,active:1}).sort([['_id', 'desc']]);
            otherTempleObject = await templeModel.find({$and : [{city : { $ne : req.body.location}},{category : req.body.category},{language: req.body.language},{active:1}]}).sort([['_id', 'desc']]);
        }
        else if(req.body.state != '' && req.body.city == ''){
            myTempleObject = await templeModel.find({category : req.body.category, state : req.body.state, language: req.body.language,active:1}).sort([['_id', 'desc']]);
            otherTempleObject = await templeModel.find({$and : [{state : { $ne : req.body.state}},{category : req.body.category},{language: req.body.language},{active:1}]}).sort([['_id', 'desc']]);
        }
        else{
            myTempleObject = await templeModel.find({category : req.body.category, state : req.body.state, city : req.body.city, language: req.body.language,active:1}).sort([['_id', 'desc']]);
            otherTempleObject = await templeModel.find({$and : [{category : req.body.category},{language: req.body.language},{active : 1}, {state : { $ne : req.body.state}}, {city : { $ne : req.body.city}}]}).sort([['_id', 'desc']]);
        }
        otherTempleList.push(otherTempleObject)
        myTempleList.push(myTempleObject)
        res.status(200).json({success : true,myList: myTempleObject, otherlist : otherTempleObject})
    },
    ...
    updateTempleStatus : function(req, res){
        try{
            jwt.verify(req.headers.token, 'bootspider', async function(err, user){
                if (err) res.status(400).json({success : false,message: err.message});
                var templeUpdate =  await templeModel.findOneAndUpdate({_id : req.body.id}, 
                    {
                        active : req.body.status
                    },{
                    new: true
    ...
    getTempleWeather : async function(req, res){
        try{
            var lat, lon;
            var templeDetails = await templeModel.findOne({_id : req.body.id}).select('city');
            request.get('http://api.openweathermap.org/geo/1.0/direct?q='+templeDetails.city+'&limit=1&appid=0a93059cba28d7c0a543058259661bc0', function (error1, response1, latLon) {
                if(!error1){
                    if(JSON.parse(latLon).length > 0){
                        lat = JSON.parse(latLon)[0].lat
                        lon = JSON.parse(latLon)[0].lon
                        request.get('https://api.openweathermap.org/data/2.5/weather?lat='+lat+'&lon='+lon+'&appid=0a93059cba28d7c0a543058259661bc0', function (error, response, body) {
                            if(!error){
                                res.status(200).json({success : true, message: JSON.parse(body)})
                            }
                            else{
                        ...
                        res.status(400).json({success : false, message: 'Error in third party api'}) 
                    }
                }
                else{
