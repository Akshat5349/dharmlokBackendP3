                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                <td style="padding-bottom: 5px; padding-left: 20px; padding-right: 20px;" align="center" valign="top" class="mainTitle">
                                                                    <h2 class="text" style="color:#000;font-family:Poppins,Helvetica,Arial,sans-serif;font-size:28px;font-weight:500;font-style:normal;letter-spacing:normal;line-height:36px;text-transform:none;text-align:center;padding:0;margin:0">Hi "Welcome"</h2>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                        ...
                                                                        <tbody>
                                                                            <tr>
                                                                                <td style="padding-bottom: 20px;" align="center" valign="top" class="description">
                                                                                    <p class="text" style="color:#666;font-family:'Open Sans',Helvetica,Arial,sans-serif;font-size:14px;font-weight:400;font-style:normal;letter-spacing:normal;line-height:22px;text-transform:none;text-align:center;padding:0;margin:0">Congratulation! You have successfully created your account at www.dharmlok.com. Please verify your email by clicking on following link.</p>
                                                                                </td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                            ...
                                                                            <tr>
                                                                                <td style="padding-top:20px;padding-bottom:20px" align="center" valign="top">
                                                                                    <table border="0" cellpadding="0" cellspacing="0" align="center">
                                                                                        <tbody>
                                                                                            <tr>
                                                                                                <td style="background-color: #e41032; padding: 12px 35px; border-radius: 50px;" align="center" class="ctaButton"> <a href="#" style="color:#fff;font-family:Poppins,Helvetica,Arial,sans-serif;font-size:13px;font-weight:600;font-style:normal;letter-spacing:1px;line-height:20px;text-transform:uppercase;text-decoration:none;display:block" target="_blank" class="text">Confirm Email</a>
                                                                                                </td>
                                                                                            </tr>
                                                                                        </tbody>
                                                                                    </table>
...
module.exports = {
    signUp : async function(req, res){
        try {
            const {name,phone,email,password,userType,typeVendor,category,social} = req.body;
           let user = await userModel.findOne({email});
           if (user) return res.status(400).json({ success : false, message: "Email Already Exists"});
           user =  new userModel({name,phone,email,password,userType,typeVendor,category, social})
    ...
    login : async function(req, res){
        try {
            ...
            if(social == 0){
                var user = await userModel.findOne({email, active : 1});
                if (user) {
                ...
                var  userFound = await userModel.findOne({email, active : 1, social : 0});{
                    if(userFound){
                        res.status(400).json({success : false,message: "User logged in using Email and Password"})
                    }
                    else{
                        let user1 = await userModel.findOne({email, active : 1});
                        if(user1) {
                            var payload = {
                                id: user1.id
                            };
                            ...
                            var sign =  new userModel({
                                name : req.body.name,
                                phone : req.body.phone,
                                email : req.body.email,
                                password : req.body.password,
                                userType : req.body.userType,
                                typeVendor : req.body.typeVendor,
                                category : req.body.category, 
                                social : req.body.social})
                                const salt = await bcrypt.genSalt(10);
                                sign.password = await bcrypt.hash(req.body.password, salt);
                                await sign.save();
                                const payload = {id: sign.id};
                                sendEmail(sign.email)
                                jwt.sign(
                                        payload,
                                        "bootspider", {
                                            expiresIn: '10d'
    ...
    AdminUserLogin : async function(req, res){
        try {
